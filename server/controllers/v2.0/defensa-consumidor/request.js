 ...
 * @param {String} cuit
 */
const getProviderByCuit = (cuit) => {
  const providers = [
    {
      cuit: '30678814357',
      nombre: 'Movistar - TELEFÓNICA MÓVILES ARGENTINA S.A.',
      domicilio: 'Av. Independencia 169. Ciudad Autónoma de Buenos Aires',
      codigo_postal: '1099',
    },
    {
      cuit: '30123456788',
      nombre: 'Movisarasa - TEST S.A.',
  ...
  const provider = providers.find((val) => {
    return val.cuit == cuit
  })
 ...
 * @param {String} query
 */
const getProvidersByQuery = (query) => {
  const providers = [
    {
      cuit: '30678814357',
      nombre: 'Movistar - TELEFÓNICA MÓVILES ARGENTINA S.A.',
  ...
  const filtersProvider = providers.filter((val) => {
    return val.nombre.toUpperCase().includes(query.toUpperCase())
  })

  return new Promise((resolve, reject) => {
    resolve(filtersProvider)
  })
}
 ...
 */
const getIssuesByClientDni = (clientDni) => {
  return new Promise((resolve, reject) => {
    resolve([
      {
        id: '1',
        titulo: 'Lorem Ipsum es simplemente el texto de relleno de las imprentas ',
      },
      {
        id: '2',
        titulo: 'Lorem Ipsum es simplemente el texto de relleno de las imprentas ',
...
const getIssueById = (idIssue) => {
  ...
  const issue = issues.find((val) => {
    return val.id == idIssue
  })
